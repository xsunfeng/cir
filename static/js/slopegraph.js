function compareTags(all_data, section_id){
...
// thanks to http://skedasis.com/d3/slopegraph/
...
var data = _to_data("myTag", "allTag", all_data)
...
for (var i = 0; i < data.length; i += 1){
	data[i].left_coord = y(data[i].left)/10;
	data[i].right_coord = y(data[i].right)/10;
}
...
var sg = d3.select('#slopegraph' + "-" + section_id)
	.append('svg:svg')
	.attr('width', WIDTH)
	.attr('height', HEIGHT);

$('#slopegraph-segment' + "-" + section_id).css(
{
  'position': 'relative',
  'left': (parseInt($('#slopegraph-segment' + "-" + section_id).parent().width())-WIDTH-40).toString() + 'px',
...
sg.selectAll('.left_labels')
	.data(data).enter().append('svg:text')
		...
		.attr('fill', 'black')
		.attr('class', 'compare-tag-nav ui label')
