define([
	...
	$('#tsd-wrapper')
		.on('click', '.refresh-list', function() {
			...
			$('#tsd-wrapper .new.question.segment').toggleClass('hidden');
			...
			e.preventDefault();
			var content = $('#tsd-wrapper .new.question textarea').val();
			var theme_id = $(".item.claim-theme-filter.selected").attr("data-id");
			if (content.length == 0) {
			...
			if ($(this).text() == 'hide comment(s)') {
				$('#tsd-thread').hide();
				$(this).text('show comment(s)');
			} else {
				$(this).parents('.question.item').removeClass('need-refresh');
				// refresh thread when expanding thread
				$('#tsd-list .expand-thread').text('show comment(s)');
				$(this).text('hide comment(s)');
				$('#tsd-thread')
	...
	module.updateQuestionList = function(theme_id) {
		var theme_id = $(".item.claim-theme-filter.selected").attr("data-id");
		_tsdUpdater({
	...
	module.newReplyAdded = function(data) {
		...
		var $question = $('#tsd-list .question.item[data-id="' + question_id + '"]');
		$question.addClass('need-refresh');
		var replies = parseInt($question.find('.reply-cnt').text());
		$question.find('.reply-cnt').text(replies + 1);
	};

	function _tsdUpdater(data) {
		$('#tsd-list').css('opacity', '0.7');
		$('#tsd-refresh-prompter').addClass('hidden');
		return $.ajax({
			url: '/api_tsd/',
			type: 'post',
			data: data,
			success: function(xhr) {
				$('#tsd-list').html(xhr.html);
				$('#tsd-list').css('opacity', '1.0');
				$('#tsd-thread').feed('init');
			},
			error: function(xhr) {
				if (xhr.status == 403) {
					Utils.notify('error', xhr.responseText);
